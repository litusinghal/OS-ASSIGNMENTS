...
void allocate_memory(Elf32_Phdr *segment, uintptr_t fault_address){  
    if(segment->p_memsz<4096){
        fragmentation+=4096-segment->p_memsz;
    }
    void* map_address =mmap((void *)fault_address ,4096,PROT_READ | PROT_WRITE | PROT_EXEC,MAP_PRIVATE | MAP_ANONYMOUS,0,0); 
